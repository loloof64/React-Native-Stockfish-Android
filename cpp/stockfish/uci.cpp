...
namespace UCI {
  ...
  void commandInit() {
      ...
      currentPos.set(StartFEN, false, &states->back(), Threads.main());
  }
  ...
  void position(istringstream& is) {
    ...
    currentPos.set(fen, Options["UCI_Chess960"], &states->back(), Threads.main());

    // Parse move list (if any)
    while (is >> token && (m = UCI::to_move(currentPos, token)) != MOVE_NONE)
    {
        states->emplace_back();
        currentPos.do_move(m, states->back());
    }
  }
  ...
  void go(istringstream& is) {
    ...
    while (is >> token)
        if (token == "searchmoves") // Needs to be the last command on the line
            while (is >> token)
                limits.searchmoves.push_back(UCI::to_move(currentPos, token));
    ...
    Threads.start_thinking(currentPos, states, limits, ponderMode);
  }
  ...
  void bench(istream& args) {

    string token;
    uint64_t num, nodes = 0, cnt = 1;

    vector<string> list = setup_bench(currentPos, args);
    ...
    for (const auto& cmd : list)
    {
        ...
        if (token == "go" || token == "eval")
        {
            ...
            position_msg << "Position: " << cnt++ << '/' << num << " (" << currentPos.fen() << ")";
    ...
    outputs.push("===========================");
    err_stats_msg    << "Total time (ms) : " << elapsed;
    outputs.push(err_stats_msg.str());

    std::stringstream message2;
    message2 << "Nodes searched  : " << nodes;
    outputs.push(message2.str());

    std::stringstream message3;
    message3 << "Nodes/second    : " << 1000 * nodes / elapsed;
    outputs.push(message3.str());
  }

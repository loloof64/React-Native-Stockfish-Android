...
extern loloof64::LockedStringQueue outputs;
  ...
  uint64_t perft(Position& pos, Depth depth) {
    ...
    for (const auto& m : MoveList<LEGAL>(pos))
    {
        ...
        if (Root) {
            ...
            message << UCI::move(m, pos.is_chess960()) << ": " << cnt;
...
void MainThread::search() {
  ...
  if (rootMoves.empty())
  {
      ...
      message3 << "info depth 0 score "
                << UCI::value(rootPos.checkers() ? -VALUE_MATE : VALUE_DRAW);
      outputs.push(message3.str());
  }
  else
  {
  ...
  if (bestThread != this) {
      ...
      message4 << UCI::pv(bestThread->rootPos, bestThread->completedDepth, -VALUE_INFINITE, VALUE_INFINITE);
      outputs.push(message4.str());
  }

  std::stringstream message5;
  message5 << "bestmove " << UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());

  if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos))
      message5 << " ponder " << UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());

  outputs.push(message5.str());
}
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          while (true)
          {
              ...
              if (   mainThread
                  ...
                  && Time.elapsed() > 3000) {
                  std::stringstream message6;
                  message6 << UCI::pv(rootPos, rootDepth, alpha, beta);
                  outputs.push(message6.str());
              }
          ...
          if (    mainThread
              ...
              std::stringstream message7;
              message7 << UCI::pv(rootPos, rootDepth, alpha, beta);
              outputs.push(message7.str());
          }
      }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (rootNode && thisThread == Threads.main() && Time.elapsed() > 3000) {
          std::stringstream message8;
          message8 << "info depth " << depth
                    << " currmove " << UCI::move(move, pos.is_chess960())
                    << " currmovenumber " << moveCount + thisThread->pvIdx;
          outputs.push(message8.str());
      }
      if (PvNode)
          (ss+1)->pv = nullptr;
